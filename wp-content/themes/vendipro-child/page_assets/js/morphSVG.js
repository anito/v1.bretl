jQuery(document).ready(function ($) {
    ...
    $(document).on('svg.loaded', function (e, id) {
        ...
        var rotation = document.querySelector("#rotational"),
                ...
                devTools, tracer, tl,
                $svg_from = '#'+svg_morph_ids['$svg_from'],
                $svg_to = '#'+svg_morph_ids['$svg_to'],
                svg_from_el = document.querySelector($svg_from),
                svg_to_el = document.querySelector($svg_to);
        ...
        console.log(svg_morph_ids)

        function start(paused) {
            
            if (!svg_from_el || !svg_to_el || !ghost) {

                throw ('Not enough SVGs to Morph')

            };
            
            // converts SVG shapes like <circle>, <rect>, <ellipse>, <plygon> or <line> into <path>
            MorphSVGPlugin.convertToPath('circle, rect, ellipse, line, polygon, polyline')

            console.log(svg_from_el)
            console.log(svg_to_el)
            
            ghost.setAttribute("d", svg_from_el.getAttribute("d"));
            ...
            var tween = TweenLite.to($svg_from, 3, {
                morphSVG: {
                    shape: $svg_to,
                    type: type,
                    origin: origin.value
            ...
            if (showHandles.checked) {
                tracer = MorphTracer.create(tween, {
                    precision: 0.5,
                    lineOpacity: 0.4,
                    lineColor: "#886ead",
                    originColor: "#9bea00"
        ...
        // hide all svgs but the start
        $('svg[id="svg"] > *:not([id="' + svg_morph_ids['$svg_from'] + '"])').css({visibility: 'hidden'});

        var showing = true;
        $("#svg").on("click", function () {
            showing = !showing;
            TweenLite.set(".controls, h1", {
            ...
            try{
                ...
                throw('Could not create TimeLine')
            }
        });
